...
def main(args):
    ...
    results = {"scores": scores, "filepaths": filepaths}
    ...
    if filepaths is not None:
        ...
        if len(filepaths) == len(scores):
            df = pd.DataFrame({"filepath": filepaths, "zcore_score": scores})
...
if __name__ == "__main__":
    ...
    parser.add_argument("--embedding", type=str, nargs="+", choices=["resnet18", "clip", "dinov2", "yolo"])
    parser.add_argument("--n_sample", type=int, default=int(1e6))
    parser.add_argument("--sample_dim", type=int, default=2)
    parser.add_argument("--no_rand_init", dest="rand_init", action="store_false", default=True)
