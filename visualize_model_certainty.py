...
def generate_model_predictions(args, pred_f):
    ...
    # Load model.
    checkpoint = torch.load(args.model_weights, map_location=args.device, weights_only=False)
    ...
    if args.split == "test":
        ...
        raise ValueError(f"{args.split} not recognized.")
    data_loader = torch.utils.data.DataLoader(
        current_data, batch_size=args.batch_size, num_workers=args.num_workers, shuffle=False, pin_memory=True
    )
...
def main(args):
    ...
    if not os.path.isfile(pred_f):
        generate_model_predictions(args, pred_f)
    ...
    fob.compute_visualization(
        dataset_fo, embeddings=predictions, num_dims=2, brain_key=f"model_certainty", verbose=True, seed=51
    )
